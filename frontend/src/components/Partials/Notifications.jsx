...
export default function Notifications({socket}){
    ...
    const handleShowNotif = () => {
        ...
        const objDiv = document.querySelector(".notifications-container");
    ...
    return(
        <>
            ...
            <ul className={`${showNotif ? "absolute block" : "hidden"} z-20 right-[10px] top-[60px] max-h-[250px] overflow-auto rounded bg-neutral-950 flex flex-col gap-2 notifications-container`}>
                    ...
                    notifications.map((notification, index) => (                    
                        <li className="flex flex-col border-b-2 border-slate-50 p-4" key={index}>
                            ...
                            <span className="text-center text-xs">
                                {new Date(notification.time).toLocaleTimeString([], { hour12: false })}
                            </span>
                        </li>  
                    ))
                }
